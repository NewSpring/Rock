    ...
    Rock.controls.groupScheduler = (function() {
        var exports = {
            ...
            initialize: function(options) {
                ...
                self.$additionalPersonIds = $(
                    '.js-resource-additional-person-ids',
                    self.$resourceList,
                );
                ...
                containers.push(
                    $control.find('.js-scheduler-source-container')[0],
                );

                // add all the occurrences (locations) as dragula containers
                var targets = $control
                    .find('.js-scheduler-target-container')
                    .toArray();
                ...
                self.resourceListDrake = dragula(containers, {
                    ...
                    moves: function(el, source, handle, sibling) {
                        if (
                            source.classList.contains(
                                'js-scheduler-source-container',
                            ) &&
                            ($(el).data('has-blackout-conflict') ||
                                $(el).data('has-requirements-conflict'))
                        ) {
                    ...
                    accepts: function(el, target, source, sibling) {
                        if (
                            target.classList.contains(
                                'js-scheduler-target-container',
                            )
                        ) {
                            var $resourceDiv = $(el);
                            var blackoutDates = $resourceDiv.data(
                                'blackout-dates',
                            );

                            var $targetOccurrence = $(target).closest(
                                '.js-scheduled-occurrence',
                            );
                            var targetOccurrenceDate = new Date(
                                $targetOccurrence
                                    .find('.js-attendanceoccurrence-date')
                                    .val(),
                            ).getTime();
                            ...
                            if (blackoutDates) {
                                ...
                                var blackdateDateList = blackoutDates.map(
                                    function(d) {
                            ...
                            var $sourceOccurrence = $resourceDiv.closest(
                                '.js-scheduled-occurrence',
                            );
                            var allowedDate = $resourceDiv.data(
                                'occurrenceDate',
                            );
                    ...
                    invalid: function(el, handle) {
                        ...
                        var isMenu = $(el).closest('.js-resource-actions')
                            .length;
                        return isMenu;
                    },
                    ...
                    .on('drop', function(el, target, source, sibling) {
                        ...
                        if (
                            ...
                            $unscheduledResource.attr(
                                'data-status',
                                'unscheduled',
                            );

                            var personId = $unscheduledResource.attr(
                                'data-person-id',
                            );

                            var additionalPersonIds = self.$additionalPersonIds
                                .val()
                                .split(',');
                            ...
                            var scheduledPersonAddPendingUrl =
                                Rock.settings.get('baseUrl') +
                                'api/Attendances/ScheduledPersonAddPending';
                            var scheduledPersonAddConfirmedUrl =
                                Rock.settings.get('baseUrl') +
                                'api/Attendances/ScheduledPersonAddConfirmed';
                            var $scheduledResource = $(el);

                            var personId = $scheduledResource.attr(
                                'data-person-id',
                            );
                            var attendanceOccurrenceId = $(target)
                                .closest('.js-scheduled-occurrence')
                                .find('.js-attendanceoccurrence-id')
                                .val();
                            var $occurrence = $(el).closest(
                                '.js-scheduled-occurrence',
                            );
                            ...
                            if (
                                ...
                                $scheduledResource.attr(
                                    'data-status',
                                    'pending',
                                );
                            }

                            var refreshAllOccurrences =
                                $scheduledResource.data(
                            ...
                            $.ajax({
                                ...
                                .done(function(scheduledAttendance) {
                                    ...
                                    if (refreshAllOccurrences) {
                                        ...
                                        self.populateScheduledOccurrence(
                                            $occurrence,
                                        );
                                    }
                                })
            ...
            removeResource: function($scheduledResource, $occurrence) {
                ...
                var attendanceId = $scheduledResource.attr(
                    'data-attendance-id',
                );
                var refreshAllOccurrences =
                    $scheduledResource.data('occurrence-date-count') > 1;

                // unschedule and repopulate ui
                var scheduledPersonRemoveUrl =
                    Rock.settings.get('baseUrl') +
            ...
            populateScheduledOccurrence: function($occurrence) {
                ...
                var attendanceOccurrenceId = $occurrence
                    .find('.js-attendanceoccurrence-id')
                    .val();
                var $schedulerTargetContainer = $occurrence.find(
                    '.js-scheduler-target-container',
                );

                var minimumCapacity = $occurrence
                    .find('.js-minimum-capacity')
                    .val();
                var desiredCapacity = $occurrence
                    .find('.js-desired-capacity')
                    .val();
                var maximumCapacity = $occurrence
                    .find('.js-maximum-capacity')
                    .val();
                var $schedulingStatusContainer = $occurrence.find(
                    '.js-scheduling-status',
                );
                var $autoschedulerWarning = $occurrence.find(
                    '.js-autoscheduler-warning',
                );
                var occurrenceDate = new Date(
                    $occurrence.find('.js-attendanceoccurrence-date').val(),
                ).getTime();
                ...
                $.get(
                    getScheduledUrl +
                        '?attendanceOccurrenceId=' +
                        attendanceOccurrenceId,
                    function(scheduledAttendanceItems) {
                        ...
                        $.each(scheduledAttendanceItems, function(i) {
                            var scheduledAttendanceItem =
                                scheduledAttendanceItems[i];

                            // add up status numbers
                            if (
                                scheduledAttendanceItem.ConfirmationStatus ==
                                'confirmed'
                            ) {
                                totalConfirmed++;
                            } else if (
                        ...
                        var $statusLight = $schedulingStatusContainer.find(
                            '.js-scheduling-status-light',
                        );

                        var totalPendingOrConfirmed =
                            totalConfirmed + totalPending;

                        if (
                            minimumCapacity &&
                            totalPendingOrConfirmed < minimumCapacity
                        ) {
                            $statusLight.attr('data-status', 'below-minimum');
                        } else if (
                            desiredCapacity &&
                            totalPendingOrConfirmed < desiredCapacity
                        ) {
                        ...
                        $schedulingStatusContainer.attr(
                            'data-original-title',
                            toolTipHtml,
                        );
                        $schedulingStatusContainer.tooltip({ html: true });

                        var confirmedPercent =
                            !progressMax ||
                            (totalConfirmed * 100) / progressMax;
                        var pendingPercent =
                            !progressMax || (totalPending * 100) / progressMax;
                        var minimumPercent =
                            !progressMax ||
                            (minimumCapacity * 100) / progressMax;
                        var desiredPercent =
                            !progressMax ||
                            (desiredCapacity * 100) / progressMax;

                        var $progressMinimumIndicator = $schedulingStatusContainer.find(
                            '.js-minimum-indicator',
                        );
                        var $progressDesiredIndicator = $schedulingStatusContainer.find(
                            '.js-desired-indicator',
                        );
                        ...
                        var $progressConfirmed = $schedulingStatusContainer.find(
                            '.js-scheduling-progress-confirmed',
                        );
                        $progressConfirmed.css({
                            width: confirmedPercent + '%',
                        });
                        $progressConfirmed
                            .find('.js-progress-text-percent')
                            .val(confirmedPercent);

                        var $progressPending = $schedulingStatusContainer.find(
                            '.js-scheduling-progress-pending',
                        );
            ...
            populateSchedulerResources: function($resourceList) {
                ...
                if (self.$additionalPersonIds.val() != '') {
                    additionalPersonIds = self.$additionalPersonIds
                        .val()
                        .split(',');
                }
                ...
                var schedulerResourceParameters = {
                    AttendanceOccurrenceGroupId: Number(
                        $('.js-occurrence-group-id', $resourceList).val(),
                    ),
                    AttendanceOccurrenceScheduleId: Number(
                        $('.js-occurrence-schedule-id', $resourceList).val(),
                    ),
                    AttendanceOccurrenceSundayDate: $(
                        '.js-occurrence-sunday-date',
                        $resourceList,
                    ).val(),
                    ResourceGroupId: $(
                        '.js-resource-group-id',
                        $resourceList,
                    ).val(),
                    GroupMemberFilterType: $(
                        '.js-resource-groupmemberfiltertype',
                        $resourceList,
                    ).val(),
                    ResourceDataViewId: $(
                        '.js-resource-dataview-id',
                        $resourceList,
                    ).val(),
                    ResourceAdditionalPersonIds: additionalPersonIds,
                };

                var $loadingNotification = $resourceList.find(
                    '.js-loading-notification',
                );
                ...
                $.ajax({
                    ...
                    .done(function(schedulerResources) {
                        ...
                        var $resourceTemplate = $(
                            '.js-unscheduled-resource-template',
                        ).find('.js-resource');
            ...
            populateResourceDiv: function($resourceDiv, schedulerResource) {
                $resourceDiv.attr(
                    'data-status',
                    schedulerResource.ConfirmationStatus,
                );
                $resourceDiv.attr('data-person-id', schedulerResource.PersonId);
                $resourceDiv.attr(
                    'data-has-scheduling-conflict',
                    schedulerResource.HasSchedulingConflict,
                );

                // entirely blacked out for all the occurrences for the selected week and schedule
                $resourceDiv.attr(
                    'data-has-blackout-conflict',
                    schedulerResource.HasBlackoutConflict,
                );

                // blacked out for some of the occurrences for the selected week and schedule, but has some dates that are not blacked out
                $resourceDiv.attr(
                    'data-has-partial-blackout-conflict',
                    schedulerResource.HasPartialBlackoutConflict,
                );
                $resourceDiv.data(
                    'blackout-dates',
                    schedulerResource.BlackoutDates,
                );

                $resourceDiv.attr(
                    'data-occurrence-date-count',
                    schedulerResource.OccurrenceDateCount,
                );

                $resourceDiv.attr(
                    'data-has-requirements-conflict',
                    schedulerResource.HasGroupRequirementsConflict,
                );
                ...
                if (schedulerResource.HasBlackoutConflict) {
                    ...
                    if (schedulerResource.HasPartialBlackoutConflict) {
                        var blackdateDates = schedulerResource.BlackoutDates.map(
                            function(d) {
                                return new Date(d).toLocaleDateString();
                            },
                        );
                        var firstBlackoutDate = blackdateDates[0];
                        var lastBlackoutDate =
                            blackdateDates[blackdateDates.length - 1];
                        resourceTooltip +=
                            ' has a blackout from ' +
                            firstBlackoutDate +
                            ' to ' +
                ...
                $resourceDiv
                    .find('.js-resource-name')
                    .text(schedulerResource.PersonName);
                if (schedulerResource.Note) {
                    $resourceDiv.addClass('has-note');
                    $resourceMeta
                        .parent()
                        .prepend(
                            '<div class="resource-note js-resource-note hide-transit">' +
                                schedulerResource.Note +
                                '</div>',
                        );
                }
                ...
                if (schedulerResource.ConflictNote) {
                    $resourceMeta.append(
                        '<span class="resource-warning hide-transit">' +
                            schedulerResource.ConflictNote +
                            '</span>',
                    );
                }
                ...
                if (schedulerResource.HasSchedulingConflict) {
                    $resourceMeta.append(
                        '<span class="resource-scheduling-conflict hide-transit" title="Scheduling Conflict"><i class="fa fa-user-clock"></i></span>',
                    );
                }

                if (schedulerResource.HasBlackoutConflict) {
                    $resourceMeta.append(
                        '<span class="resource-blackout-status hide-transit" title="Blackout"><i class="fa fa-user-times"></i></span>',
                    );
                }

                if (schedulerResource.HasPartialBlackoutConflict) {
                    $resourceMeta.append(
                        '<span class="resource-partial-blackout-status hide-transit" title="Partial Blackout"><i class="fa fa-user-clock"></i></span>',
                    );
                }

                if (schedulerResource.HasGroupRequirementsConflict) {
                    $resourceMeta.append(
                        '<span class="resource-requirements-conflict hide-transit" title="Group Requirements Not Met"><i class="fa fa-exclamation-triangle"></i></span>',
                    );
                }

                if (schedulerResource.LastAttendanceDateTime) {
                    $resourceMeta.append(
                        '<span class="resource-lastattendeddate hide-transit" title="Last Attended" data-datetime="' +
                            schedulerResource.LastAttendanceDateTime +
                            '">' +
                            schedulerResource.LastAttendanceDateTimeFormatted +
                            '</span>',
                    );
                }
                ...
                if (schedulerResource.IsAlreadyScheduledForGroup != null) {
                    $resourceDiv.attr(
                        'data-is-scheduled',
                        schedulerResource.IsAlreadyScheduledForGroup,
                    );
                }
                ...
                if (schedulerResource.AttendanceId) {
                    $resourceDiv.attr(
                        'data-attendance-id',
                        schedulerResource.AttendanceId,
                    );
                }
            },
            /**  */
            initializeEventHandlers: function() {
                ...
                self.$groupScheduler.on(
                    'click',
                    '.js-markconfirmed, .js-markdeclined, .js-markpending, .js-resendconfirmation',
                    function(a, b, c) {
                        ...
                        if ($(this).hasClass('js-markconfirmed')) {
                            ...
                            scheduledPersonUrl =
                                Rock.settings.get('baseUrl') +
                                'api/Attendances/ScheduledPersonSendConfirmationEmail';
                        } else {
                            return;
                        }
                        ...
                        $.ajax({
                            ...
                            .done(function() {
                                ...
                                var $occurrence = $resource.closest(
                                    '.js-scheduled-occurrence',
                                );
                                self.populateScheduledOccurrence($occurrence);
                            })
                ...
                $(window).mousemove(function(e) {
                    if (self.resourceListDrake.dragging) {
                        ...
                        var editorMousePositionProportion =
                            editorMouseY / editorScrollHeight;
                        if (editorMousePositionProportion > 0.9) {
                            editorScrollLevel += 20;
                            $editorScrollWindow.scrollTop(editorScrollLevel);
                        } else if (
                            editorMousePositionProportion < 0.1 &&
                            editorScrollLevel != 0
                        ) {
                            editorScrollLevel -= 20;
                            $editorScrollWindow.scrollTop(editorScrollLevel);
